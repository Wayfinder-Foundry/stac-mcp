...
def test_cached_search_expiry_and_refresh():
    ...
    def _search_a(**_kw):
        return FakeSearch([{"id": "a"}])

    client._client = SimpleNamespace(search=_search_a)
    items1 = list(client._cached_search(collections=["c1"], limit=1))
    ...
    def _search_b(**_kw):
        return FakeSearch([{"id": "b"}])

    client._client = SimpleNamespace(search=_search_b)
    items2 = list(client._cached_search(collections=["c1"], limit=1))
